...
# get taxid with fallback by progressively shortening the name
resolve_taxid_with_fallback <- function(name, sql_db) {
  ...
  for (k in seq.int(length(tokens), 1)) {
    ...
    if (!is.na(id)) return(as.character(id))  # 统一字符
  }
  "Unknown"
}

# safely pick first non-empty value or "Unknown"
...
augment_with_taxonomy_offline_rawmatch <- function(df,
  ...
  resolved_ids <- vapply(
    all_names_raw,
    function(x) resolve_taxid_with_fallback(x, sql_db),
...
metorigindb_database_count <- 
  ...
  mutate(
    ...
    Unknown_count = str_count(bacteria_ncbi_id, "Unknown") + 1)
...
write.csv(df, "~/tidymass/metorigindb/2_data/41_MetOriginDB/unknown_bacteria.csv", row.names = FALSE)

unknown_bacteria_id <- read_csv("~/tidymass/metorigindb/2_data/41_MetOriginDB/unknown_bacteria_id.csv")

# save the results
save(metorigindb_database, file = "~/tidymass/metorigindb/2_data/41_MetOriginDB/metorigin_database.rda", compress="xz")
